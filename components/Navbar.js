...
export default function Navbar() {
  ...
  return (
    <nav ref={navRef} style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 1000, background: '#1a1612', height: 'var(--nav-h)', display: 'flex', alignItems: 'center', padding: '0 32px', borderBottom: '1px solid #2a2520' }}>
      <div style={{ maxWidth: 'var(--max-w)', margin: '0 auto', width: '100%', display: 'flex', alignItems: 'center' }}>
          ...
          {MENU.map(item => (
            <div key={item.label} style={{ position: 'relative' }}>
              {item.altlar.length > 0 ? (
                <button onClick={() => setAcik(acik === item.label ? null : item.label)} style={{ padding: '7px 12px', borderRadius: '8px', border: 'none', background: acik === item.label ? '#2a2520' : 'transparent', color: acik === item.label ? '#fff' : '#9a9a9a', cursor: 'pointer', fontSize: '14px', fontFamily: 'var(--font-body)', display: 'flex', alignItems: 'center', gap: '4px' }}>
                ...
                <Link href={item.url} onClick={() => setAcik(null)} style={{ padding: '7px 12px', borderRadius: '8px', color: '#9a9a9a', fontSize: '14px', fontFamily: 'var(--font-body)', display: 'inline-block' }}>{item.label}</Link>
              )}
              {item.altlar.length > 0 && acik === item.label && (
                <div style={{ position: 'absolute', top: 'calc(100% + 12px)', left: 0, background: '#1c1914', borderRadius: '12px', minWidth: '240px', border: '1px solid #2a2520', boxShadow: '0 12px 40px rgba(0,0,0,0.4)', overflow: 'hidden', zIndex: 100 }}>
                  <Link href={item.url} onClick={() => setAcik(null)} style={{ display: 'block', padding: '12px 16px', fontSize: '12px', color: '#4a4540', borderBottom: '1px solid #2a2520', fontWeight: 700, letterSpacing: '1px', textTransform: 'uppercase' }}>Tüm {item.label} →</Link>
                  {item.altlar.map(alt => (
                    <Link key={alt.label} href={alt.url} onClick={() => setAcik(null)} style={{ display: 'flex', alignItems: 'flex-start', gap: '10px', padding: '12px 16px' }}
        ...
        <Link href="/iletisim" onClick={() => setAcik(null)} style={{ padding: '9px 20px', borderRadius: '8px', background: 'var(--orange)', color: '#fff', fontSize: '14px', fontWeight: 600, fontFamily: 'var(--font-body)' }}>İletişim</Link>
      </div>
    </nav>
  )
}
